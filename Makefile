...
$(BUILT)/util.o: $(SRC)/util.cpp $(SRC)/util.h
	g++-7 $(SRC)/util.cpp -c $(CFLAGS) -o $(BUILT)/util.o -ljpeg -lpng -fPIC

$(BUILT)/image_io.o: $(SRC)/image_io.cpp $(SRC)/image_io.h
	g++-7 $(SRC)/image_io.cpp -c $(CFLAGS) -o $(BUILT)/image_io.o -ljpeg -lpng -fPIC

$(BUILT)/serialize.o: $(SRC)/serialize.cpp $(SRC)/serialize.h
	g++-7 $(SRC)/serialize.cpp -c $(CFLAGS) -o $(BUILT)/serialize.o -ljpeg -lpng -fPIC
...
$(BUILT)/image_util.o: $(SRC)/image_util.cpp $(SRC)/image_util.h
	g++-7 $(SRC)/image_util.cpp -c $(CFLAGS) -o $(BUILT)/image_util.o -fPIC

$(BUILT)/io_util.o: $(SRC)/io_util.cpp $(SRC)/io_util.h
	g++-7 $(SRC)/io_util.cpp -c $(CFLAGS) -o $(BUILT)/io_util.o -fPIC

$(BUILT)/python_binding.o: $(SRC)/python_binding.cpp
	g++-7 $(SRC)/python_binding.cpp -c $(CFLAGS) -o $(BUILT)/python_binding.o -fPIC `python3 -m pybind11 --includes`
...
cuda_raytrace_java.so: $(BUILT)/image_io.o $(BUILT)/image_util.o $(BUILT)/raytracer.o $(BUILT)/serialize.o $(BUILT)/util.o $(BUILT)/java_binding.o $(BUILT)/io_util.o $(BUILT)/serialize.o
	g++-7 $(CFLAGS) -lc $(BUILT)/image_io.o $(BUILT)/image_util.o $(BUILT)/raytracer.o $(BUILT)/util.o $(BUILT)/io_util.o $(BUILT)/serialize.o $(BUILT)/java_binding.o -lcudart  -L/usr/local/cuda-8.0/lib64/ -shared -fPIC -o cuda_raytrace_java.so -I$(jni_library) -I$(jni_library)/linux ${LFLAGS}
